...
class Model:
    ...
    def __init__(
        ...
        def batchify(
            self: "Model", embedded: torch.Tensor, netchunk: int
        ) -> torch.Tensor:
            return torch.cat(
                [
        ...
        def run(
            ...
            if netchunk is None:
                outputs_flat: torch.Tensor = self.model(embedded)
            else:
                outputs_flat: torch.Tensor = self.batchify(embedded)
        ...
        def raw2outputs(
            ...
            raw2alpha: t.Callable = lambda raw, dists, act_fn=F.relu: 1.0 - torch.exp(-act_fn(raw) * dists)
            ...
            weights: torch.Tensor = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), 1.0 - alpha + 1e-10], -1), -1)[:, :-1]
            rgb_map: torch.Tensor = torch.sum(weights[..., None] * rgb, -2)

            depth_map: torch.Tensor = torch.sum(weights * z_vals, -1)
            disp_map: torch.Tensor = 1.0 / torch.max(1e-10 * torch.ones_like(depth_map), depth_map / torch.sum(weights, -1))
